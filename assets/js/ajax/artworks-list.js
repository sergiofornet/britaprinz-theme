function artworksList (artworks) {
	...
	if ( typeof artworks === 'object' ) {
		...
		artworks.forEach( (element) => {
			const {featured_techniques, other_techniques} = element.artwork_techniques;
			html += `
			<li class="artwork" key="artwork-${ element.slug }">
				...
				<div class="artwork__stuff">
					...
					<div class="artwork__techniques">
						${ featured_techniques.map(technique => `<a href="${technique[1]}">${technique[0]}</a>`).join('\n') }
						${ other_techniques && `<span>${other_techniques}</span>` }
					</div>
